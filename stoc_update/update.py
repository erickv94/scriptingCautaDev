...
response = requests.request("POST", url_sku_by_refid, data=payload, headers=headers)    
...
for key, value in skus_filtered.items():
    sku_id=key
    url_update_invetory="https://vetro.vtexcommercestable.com.br/api/logistics/pvt/inventory/skus/{}/warehouses/{}".format(sku_id,warehouse)
    payload = {"unlimitedQuantity":False,"quantity":value,"dateUtcOnBalanceSystem": None}
    payload=json.dumps(payload)
    response = requests.request("PUT", url_update_invetory, data=payload, headers=headers)
    print(" -- SkuId: {} updated with {} units on stock, url used {}".format(key,value,url_update_invetory))
